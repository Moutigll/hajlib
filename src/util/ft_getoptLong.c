...
/*   Updated: 2026/02/25 18:14:37 by moutig           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/hstring.h"
#include "../../include/hgetopt.h"
	...
	const tFtLongOption	*opts)
{
	...
	while (opts[i].name)
	{
		if (ft_strcmp(opts[i].name, name) == 0)
			return (&opts[i]);
		i++;
	}
	return (NULL);
	...
	const tFtLongOption **b)
{
	...
	while (opts[i].name)
	{
		if (ft_strncmp(opts[i].name, name, len) == 0)
		{
			if (count == 0)
				*a = &opts[i];
			else if (count == 1)
